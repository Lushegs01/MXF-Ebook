  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 h-full flex flex-col justify-center bg-[#0F0F0F]">
      <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 shadow-2xl relative overflow-hidden">
        {/* Glow effect */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-[#F26F21]/20 blur-[50px] rounded-full pointer-events-none" />

        <Crown className="h-12 w-12 text-[#F26F21] mb-6" />
        ...
        <div className="space-y-4 mb-8 relative z-10">
          {FEATURES.map((feature, i) => (
            <div key={i} className="flex items-center gap-3">
              <div className="h-6 w-6 rounded-full bg-[#F26F21]/10 flex items-center justify-center">
                <Check className="h-3 w-3 text-[#F26F21] font-bold" />
              </div>
        ...
        {isPremium ? (
          <div className="w-full h-14 bg-[#F26F21]/10 border border-[#F26F21]/30 text-[#F26F21] rounded-2xl font-bold text-lg flex items-center justify-center gap-2 relative z-10">
          ...
          <button
            ...
            className="w-full h-14 bg-[#F26F21] text-black rounded-2xl font-bold text-lg hover:bg-[#F26F21]/90 transition-colors relative z-10 shadow-[0_0_20px_rgba(242,111,33,0.2)] disabled:opacity-60"
          >
